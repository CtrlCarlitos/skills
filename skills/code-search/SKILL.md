---
name: code-search
description: Pick the right code-search tool before you grep or open a file. Escalates graft (repo graph) > serena (LSP symbols) > rg (ripgrep; in Claude Code the Grep tool) > grep, gated on what is actually built and what file types are involved. Use it whenever a task starts with "where is", "how does X work", "who calls", "find all", "every occurrence", "what does this file expose", or whenever you are about to run grep, rg, find, or a code-graph or LSP query to locate something, even when the repo's instructions say to use one specific tool first. Also use it the moment a semantic tool (graft, serena) returns nothing, before re-asking it.
---
